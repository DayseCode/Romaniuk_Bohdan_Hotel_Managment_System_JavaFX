...
public class ManagerCheckIn extends DBConnection implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        // Оновлення вмісту комбо-боксу
        updateChoiceBox();
        TABLEROW.clear();
        // Ініціалізація стовпців таблиці клієнтів
        nameCol.setCellValueFactory(new PropertyValueFactory<ManagerCustomerTable, String>("Name"));
        ...
        showCustomerTable();
        // Додавання обробника подій для вибору номера кімнати
        roomChoiceBox.setOnAction(this::setRoomInfo);
    }

    // Встановлення інформації про номер кімнати при виборі
    ...
    // Оновлення вмісту комбо-боксу номерів кімнат
    ...
    // Відображення інформації про клієнтів у таблиці
    ...
    // Обробник події вибору клієнта з таблиці
    ...
    // Обробник події для здійснення операції Check-in
    public void checkInButton(ActionEvent actionEvent) throws SQLException {
        // Отримання даних про клієнта та номер кімнати
        ...
        if (name.isEmpty() || RoomNo.equals("null") || CheckInDate.equals("null")) {
            ...
            // Виконання SQL-запиту для здійснення операції Check-in
            ...
            try {
                ...
                // Оновлення статусу номера після Check-in
            ...
            // Оновлення вмісту комбо-боксу та очищення полів вводу
            updateChoiceBox();
            clearTextFields();
        }
    }
